...
class InstallGUI:
    ...
    def get_sizes(self):
        ...
        self.total_size_kb = (
            self.theme_size_kb
            + self.icons_size_kb
            + self.cursors_size_kb
            + self.background_size_kb
            + self.sounds_size_kb
            + self.fonts_size_kb
        )
    ...
    def next_clicked(self, button):
        ...
        if next_button.get_label() == "Finish":
            print("Install Completed! Enjoy Copland98!")
            Gtk.main_quit()
            return False
        ...
        for from_file in self.copy_files.items():
            ...
            if isinstance(self.copy_files[from_file], str):
                ...
                if from_file == "install_theme" and self.copy_files["install_theme"]:
                    ...
                    self.xfconf_query("xfwm4", "/general/title_font", "Pokemon GB Regular 7")
                ...
                # LOL this is a lie we don't have to do anything
                    ...
                    from_file == "terminal_colors"
                    and self.copy_files["terminal_colors"]
                ):
    ...
    def xfconf_query(self, channel, prop, new_value):
        ...
        try:
            xfconf_query_path = subprocess.check_output(
                ["which", "xfconf-query"]
            ).strip()
        except:
            print(
    ...
    def set_install_theme(self, toggle):
        if self.install_theme:
            ...
            self.remaining_size.set_label(
                "{:.0f} k".format(
                    int(self.available_kb - self.total_size_kb - self.theme_size_kb)
                )
            )
            self.total_size.set_label("{} k".format(self.total_size_kb))
        else:
    ...
    def set_install_icons(self, toggle):
        if self.install_icons:
            ...
            self.remaining_size.set_label(
                "{:.0f} k".format(
                    int(self.available_kb - self.total_size_kb - self.icons_size_kb)
                )
            )
            self.total_size.set_label("{} k".format(self.total_size_kb))
        else:
    ...
    def set_install_cursors(self, toggle):
        if self.install_cursors:
            ...
            self.remaining_size.set_label(
                "{:.0f} k".format(
                    int(self.available_kb - self.total_size_kb - self.cursors_size_kb)
                )
            )
            self.total_size.set_label("{} k".format(self.total_size_kb))
        else:
    ...
    def set_install_sounds(self, toggle):
        if self.install_sounds:
            ...
            self.remaining_size.set_label(
                "{:.0f} k".format(
                    int(self.available_kb - self.total_size_kb - self.sounds_size_kb)
                )
            )
            self.total_size.set_label("{} k".format(self.total_size_kb))
        else:
    ...
    def set_install_fonts(self, toggle):
        if self.install_fonts:
            ...
            self.remaining_size.set_label(
                "{:.0f} k".format(
                    int(self.available_kb - self.total_size_kb - self.fonts_size_kb)
                )
            )
            self.total_size.set_label("{} k".format(self.total_size_kb))
        else:
